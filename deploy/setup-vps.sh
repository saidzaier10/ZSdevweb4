...
echo ""
echo "  7. Planifier backup + retention (cron) :"
echo "     (crontab -l 2>/dev/null; echo '0 2 * * * cd $APP_DIR && make backup-db >> /var/log/zsdevweb-backup.log 2>&1'; echo '30 2 * * * cd $APP_DIR && make prune-backups DAYS=7 >> /var/log/zsdevweb-backup.log 2>&1') | crontab -"
